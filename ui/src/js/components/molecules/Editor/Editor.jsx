...
import {
  createGraphFromDAG,
  updateGraphVulnerabilities,
  findClosestLink,
  recalculateVulnerabilityIndices,
} from "./DAG.js";
...
class Editor extends React.Component {
  ...
  saveToLocalStorage() {
    ...
    window.localStorage.setItem(
      this.paperId + "graph_" + this.props.currGraph.label,
  ...
  componentDidMount() {
    ...
    this.paper = new joint.dia.Paper({
      ...
      interactive:
        this.props.interactive === undefined ? true : this.props.interactive,
    ...
    this.graph.on("change:position", (cell, position, options) => {
      const isVuln =
        cell.get("role") === "vulnerability" ||
        cell.id.toString().includes("vulnerability");

      // Si l'utilisateur déplace à la main une vulnérabilité (sans le flag autoUpdate)
      if (isVuln && (!options || !options.autoUpdate)) {
        cell.set("isManual", true);
      }

      // On recalcule l'alignement automatique uniquement pour celles qui ne sont pas débrayées
      updateGraphVulnerabilities(this.graph);
    });
    this.graph.on("change:vertices", () => {
      updateGraphVulnerabilities(this.graph);
    });
    ...
    this.paper.on("element:pointerup", (cellView) => {
      ...
      if (isVuln) {
        // On cherche s'il y a une flèche à moins de 60 pixels de l'endroit du drop
        const nearbyLink = findClosestLink(cell, this.graph, 60);

        if (nearbyLink) {
          // Effet Aimant : On réactive le mode automatique fixé à cette flèche
          cell.set("associatedLink", nearbyLink.id);
          cell.set("isManual", false);
        } else {
          // Si lâché trop loin, il reste autonome et perd ses indexations de ligne
          if (cell.get("isManual")) {
            cell.unset("vulnerabilityIndex");
            cell.unset("totalVulnerabilities");
          }
        }

        // Nettoyage général et réorganisation des espaces partagés sur les flèches
        recalculateVulnerabilityIndices(this.graph);
        updateGraphVulnerabilities(this.graph);
      }
    });
    ...
    if (this.props.interactive === undefined ? true : this.props.interactive) {
      this.paper.on("cell:contextmenu", (elementView, e, x, y) =>
      ...
      this.paper.on(
        "element:sizeSelector:pointerdown",
        this.beginElementResize,
      );
    }
  }
  ...
  attachTools(link) {
    ...
    var customToolsView = new joint.dia.ToolsView({
      tools: [verticesTool, segmentsTool, boundaryTool, removeButton],
    });
  ...
  initGraph() {
    this.graph.attributes.cells.models.map((model) => {
      if (model.attributes.type === "coras.defaultLink") {
        ...
        const sourceElem = model.getSourceElement();
        const targetElem = model.getTargetElement();

        if (sourceElem && targetElem) {
          const sourceRole =
            sourceElem.attributes.role || sourceElem.get("role") || "";
          const targetRole =
            targetElem.attributes.role || targetElem.get("role") || "";

          let relation = "leads_to";

          if (
            sourceRole.includes("human_threat") ||
            sourceRole.includes("non_human_threat") ||
            sourceRole === "threat_source"
          ) {
            relation = "initiates";
          } else if (targetRole.includes("asset")) {
            relation = "impacts";
          } else if (sourceRole === "treatment") {
            relation = "treats";
          } else if (sourceRole === "indicator") {
            relation = "indicates";
          } else if (
            targetRole === "law" ||
            (targetElem && targetElem.attributes.type === "coras.legalElement")
          ) {
            relation = "is_breached";
          }

          this.visualizeRelation(model, relation);
          model.attributes.relation = relation;
        }
      }
    });
  }
  ...
  visualizeRelation(link, relation) {
    ...
    switch (relation) {
      ...
      case "impacts":
        arrowheadShape =
          "M 8 -5 L -2 0 L 8 5 M -2 0 L 18 0 M 18 -5 L 8 0 L 18 5";
        fill = "none";
        break;
      case "treats":
    ...
    link.attr("line/targetMarker", targetMarkerAttr);
    return link;
  }
  ...
  handleScroll(cellView, e, x, y, delta) {
    ...
    if (delta > 0) {
      const newX =
        currentScale.sx * scaleFactor > 5
          ? currentScale.sx
          : currentScale.sx * scaleFactor;
      const newY =
        currentScale.sy * scaleFactor > 5
          ? currentScale.sy
          : currentScale.sy * scaleFactor;
      this.paper.scale(newX, newY);
    } else if (delta < 0) {
      const newX =
        currentScale.sx / scaleFactor < 0.52
          ? currentScale.sx
          : currentScale.sx / scaleFactor;
      const newY =
        currentScale.sy / scaleFactor < 0.52
          ? currentScale.sy
          : currentScale.sy / scaleFactor;
      this.paper.scale(newX, newY);
    }
      ...
      if (!this.state.movingLinks) {
        ...
        _.each(cellView.model.getEmbeddedCells(), (child) => {
          ...
          for (let i = 0; i < temp.length; i++) {
            if (
              arr.findIndex((element) => element.cid === temp[i].cid) === -1
            ) {
              arr.push(temp[i]);
            }
          }
  ...
  embedElement(cellView, evt, x, y) {
    ...
    if (cell.attributes.type === "coras.defaultLink") {
      ...
      if (
        cell.getTargetElement() === null ||
        cell.getSourceElement().cid === cell.getTargetElement().cid
      ) {
        cell.remove();
        return;
      }
      ...
      const sourceElem = cell.getSourceElement();
      const targetElem = cell.getTargetElement();

      if (targetElem && targetElem.attributes.type === "coras.legalElement") {
        this.visualizeRelation(cell, "is_breached");
        this.attachTools(cell);
        cell.attributes.relation = "is_breached";
        if (!cell.label(0))
          cell.label(0, {
            attrs: { text: { text: "" } },
            position: { offset: -10 },
    ...
    if (cellViewsBelow.length) {
      ...
      if (
        cellViewBelow &&
        cellViewBelow.model.get("parent") !== cell.id &&
        cellViewBelow.model.attributes.role == "stakeholder"
      ) {
  ...
  resizeElement(cellView, e, x, y) {
    if (!this.props.cellResizing) {
      ...
      if (
        !cellView.model.get("embeds") ||
        !cellView.model.get("embeds").length
      ) {
        return;
      }
  ...
  movePaper(e, x, y) {
    if (this.state.paperMove.moving) {
      ...
      this.paper.translate(
        tx + (x - this.state.paperMove.x),
        ty + (y - this.state.paperMove.y),
      );
    }
  }
  ...
  onHover(cellView, evt) {
    //console.log(cellView);
    if (
      cellView.model.attributes.type === "coras.roundRectElement" ||
      cellView.model.attributes.type === "coras.riskElement"
    ) {
      //WIP
      var cell = cellView.model;
  ...
  loadGraphFromFile(e) {
    ...
    if (filePath.files && filePath.files[0]) {
      reader.addEventListener(
        "load",
        (e) => this.loadGraphFromJSON(JSON.parse(e.target.result)),
        { once: true },
      );
  ...
  getSvg() {
    ...
    // Fix svg size
    let search =
      /(<svg xmlns="\S*" xmlns:xlink="\S*" version="\S*" id="\S*" width=)\S*( height=)\S*(>)/gm;
  ...
  cellToolHandleMoved(e) {
    ...
    const newHeight =
      pageY -
      (this.props.cellTool.position.y + this.props.cellToolHeight) +
      this.props.cellToolHeight;

    if (this.props.cellTool.handleHeld)
      this.props.cellHandleMoved(this.props.cellToolWidth, newHeight);
  }
      ...
      <div className="editor-wrapper">
        ...
        {this.props.elementEditor.visible ? (
          <ElementEditor
            ...
            perspectiveOnChange={this.props.elementEditorChangePerspective}
          />
        ) : null}
        {this.props.infoBox.visible ? <InfoBox /> : null}
        <div
        ...
        {this.props.interactive || this.props.interactive === undefined ? (
          <EditorTool toolDefinitions={ToolDefinitions} paper={this.paper} />
        ) : null}
      </div>
    );
  }
...
export default connect(
  ...
  (dispatch) => ({
    elementRightClicked: (element, graph) =>
      dispatch(ElementRightClicked(element, graph)),
    elementDoubleClicked: (element, event) =>
      dispatch(ElementDoubleClicked(element, event)),
    ...
    elementEditorChangeHeight: (height) =>
      dispatch(ElementChangeHeight(height)),
    ...
    elementEditorChangePerspective: (perspective) =>
      dispatch(ElementChangePerspective(perspective)),
    elementDropped: (graph, pageX, pageY) =>
      dispatch(ToolElementRelease(graph, pageX, pageY)),
    clearClicked: (e) => dispatch(MenuClearClicked(e)),
    clearConfirmed: () => dispatch(MenuClearConfirmed()),
    cellClicked: (x, y, width, height) =>
      dispatch(CellClicked(x, y, width, height)),
    cellHandleClicked: (handle) => dispatch(CellHandleClicked(handle)),
    cellHandleReleased: () => dispatch(CellHandleRelased()),
    cellHandleMoved: (width, height) =>
      dispatch(CellHandleMoved(width, height)),
    clearGraph: (label) => dispatch(ClearGraph(label)),
    setGraph: (label, graph, scale, position) =>
      dispatch(SetGraph(label, graph, scale, position)),
    ...
    toggleInfoBox: (pos, bool, category, id) =>
      dispatch(ToggleInfoBox(pos, bool, category, id)),
    setEditorPosition: (pos) => dispatch(SetEditorPosition(pos)),
    setModalPosition: (pos) => dispatch(SetModalPosition(pos)),
    elementChangeIndicatorType: (indicatorType) =>
      dispatch(ElementChangeIndicatorType(indicatorType)),
  }),
  null,
  { forwardRef: true },
)(Editor);
